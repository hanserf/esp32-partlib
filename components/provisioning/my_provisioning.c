                          ...
                          int32_t event_id, void* event_data)
{
	...
	//set_webserver_ip(event_data);
        ESP_ERROR_CHECK((xSemaphoreGive(wifi_sem) == pdPASS)? ESP_OK : ESP_FAIL);
...
void setup_provisioning(){
   /* Initialize NVS partition */
    ...
    StaticSemaphore_t self_test_sem;
    wifi_sem  = xSemaphoreCreateBinaryStatic(&self_test_sem);
    ESP_ERROR_CHECK((wifi_sem != NULL)? ESP_OK : ESP_FAIL);
    ...
    /*
        Clean up all resources allocated by provisioning. 
    */
    vTaskDelay(pdMS_TO_TICKS(100));
    ESP_ERROR_CHECK(esp_event_handler_unregister(WIFI_PROV_EVENT, ESP_EVENT_ANY_ID, &event_handler));
    if(!provisioning_valid){
        /*Provisioning yielded no connection to outside world. Reprovisioning required.*/
        esp_restart();
    }    
    ESP_LOGI(TAG,"Self test complete, we have connection to the outside world. \nEverything WIFI is up");
   
}


esp_err_t prov_data_handler(uint32_t session_id, const uint8_t *inbuf, ssize_t inlen,
                                          uint8_t **outbuf, ssize_t *outlen, void *priv_data)
{
    if (inbuf) {
        ESP_LOGI(TAG, "Received data: %.*s", inlen, (char *)inbuf);
